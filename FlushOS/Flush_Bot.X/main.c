...
void __attribute__((interrupt, no_auto_psv)) _T1Interrupt(void) {
    ...
    const float Kp_vx = 3.0; // 4.0
    const float Ki_vx = 0.01; // 0.1
    const float Kd_vx = 0.01; // 2.5

    const float Kp_py = 0.0;
    //    const float Ki_py = 0.0;
    //    const float Kd_py = 0.0;

    const float Kp_vy = 3.0; // 4.0
    const float Ki_vy = 0.01; //0.15
    const float Kd_vy = 0.01; // 2.5
    ...
    Output_velcont[0] = 500 + (Kp_vx * Error_velcont[0]) + (Ki_vx * SumError_velcont[0]) + (Kd_vx * (Error_velcont[0] - PreError_velcont[0]));
    // End
    ...
    Output_velcont[1] = 500 + (Kp_vy * Error_velcont[1]) + (Ki_vy * SumError_velcont[1]) + (Kd_vy * (Error_velcont[1] - PreError_velcont[1]));
    //End
...
void init_Timer() {
    ...
    PR1 = 5000; //set 500Hz Timer interrupt 
